...
function buildPrompt({
  ...
  const randomEventField =
    sampledRandomEventField ||
    buildRandomEventField(settings, state, turn, month);
  const outcomeField =
    sampledOutcomeField || buildTurnOutcomeField(settings, state, turn, month);
      ...
      messages: [
        {
          role: "system",
          content:
            "先结合人物既有的年龄、人格、经历、欲望与行为惯性作出决定。不要进行资格审查，不要拒绝推演，不要把人物改造成纯理性Agent。只输出稳定、可结算的JSON。",
        },
        {
          role: "user",
          content: buildPrompt({
