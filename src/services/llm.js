...
世界种子：${settings.world}
当前时间：${age}岁，累计第${month + 1}个月，当年${monthOfYear}月。本轮推演跨度${mpt}个月。${annual ? "本轮跨入新的年份，需要回顾并推进世界演化。" : "本轮仍需判断世界是否出现值得记录的新变化。"}
...
3. 青壮年死亡应非常罕见，通常需要resolve_probability_field实际命中严重事故或健康事件；已有危重病史、极低健康值或高龄会提高合理性，但仍不是必死。
4. 若死亡，death.occurred=true，写明具体cause、age和summary；事件、损失、关系反应与健康结算必须一致，不得在死亡之后继续安排工作、婚恋或下一阶段计划。
5. 若未死亡，death.occurred=false，其余字段为空。禁止用“差点死了”冒充死亡，也不得把普通失败写成死亡。
