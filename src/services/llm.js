...
function buildPrompt({
  ...
  const lifeStageField =
    sampledLifeStageField || { stage: "当前人生阶段", events: [] };
  ...
  const worldState = sampledWorldState || buildWorldState(settings, month);
  const lifeDomainField =
    sampledLifeDomainField || buildLifeDomainField(settings, logs, age);
...
【宏观世界状态——外部事实会穿透到普通人的生活】
${JSON.stringify(worldState)}
1. active events不是新闻装饰。能源冲击必须通过油价、交通、制造、取暖或通胀传导；房地产下行必须通过房价预期、建筑链就业、地方财政、按揭和家庭财富效应传导。
2. 宏观事件不代表每个人都立刻受损。职业、地区、资产、债务和消费结构决定暴露程度；AI浪潮也应同时创造新机会并挤压重复岗位。
3. 本轮事件、财务交易、风险变化和年度世界变化必须与worldState一致。拥有房产时才结算房价变化；没有开车、制造业或高能源消费时，油价影响应更间接。
...
3. 技能、运动、智力、天赋、财商和人格同样按模型区间与乘数生效；智力影响复杂问题和学习迁移，天赋影响适配领域的成长上限，财商影响消费、负债、投资和骗局识别，但都不能消除环境与运气。
4. 当前身高、成年最终身高和动态身材会影响一部分人的第一吸引、亲密偏好，以及模特、表演、体育、体力劳动等少数职业的适配；偏好因NPC而异，不能用单一审美把它写成择偶或职业的必然判决。
5. 多项能力应交互而非相加，例如高颜值但低技能不应自动成为成功网红，高智力但低自律也可能长期拖延，高财商也可能遇到不可预测的市场损失。
...
【亲密关系表达边界】
1. 成年角色可以直接写明吸引、性欲、是否发生性关系、频率变化、满意或不适、边界协商与关系后果，不必用空泛暗语回避成年人的真实生活。
2. 不描写生殖器细节、具体性行为步骤或以刺激为目的的色情过程；重点仍是人物动机、同意、关系动态、健康风险和后果。
3. 未满18岁的角色只允许年龄适当的暗恋、约会、亲吻、边界、困惑和情感后果，禁止任何露骨性描写；不得删掉其人生经历，但要用非露骨方式完整表达发生了什么及其影响。
...
export async function callSimulator({ apiKey, endpoint, model }, payload) {
  ...
  const sampledWorldState = buildWorldState(payload.settings, payload.month);
...
3. 性格成因、童年/中学经历、兴趣与愿望必须互相支持，并能解释上述数值组合；智力、天赋和财商要表现为具体能力或局限，不能只换同义词复述分数。
4. 不足6岁时childhood必须为空；不足12岁时school必须为空。性格和兴趣也必须符合当前年龄，例如婴幼儿不应拥有职业化技能、投资爱好或成人愿望。
5. 最终身高不可修改；身材只作为初始状态，可在后续因饮食、运动、疾病和生活方式动态变化。
